...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            return MoEBlockForOnnxExport.apply(
                hidden_states,
                router_logits,
                batch_size,
                sequence_length,
                ...
                self.num_experts,
                self.hidden_act,
                int(self.experts[0].w1.weight.shape[0].item()),
                torch.stack([expert.w1.weight.T for expert in self.experts],dim=0),
                torch.stack([expert.w2.weight.T for expert in self.experts],dim=0),
                #torch.stack([expert.w3.weight.T for expert in self.experts]),
            ), router_logits
